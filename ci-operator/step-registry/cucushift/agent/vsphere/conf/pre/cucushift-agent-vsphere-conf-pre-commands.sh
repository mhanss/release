...
for (( node=0; node < ${#all_hostnames[@]}; node++)); do
  ...
  echo "Creating IPv4 REVERSE DNS entry for ${all_hostnames[$node]}"
  reversed_ipv4=$(echo "${all_ipv4Addresses[$node]}" | awk -F. '{print $4"."$3"."$2"."$1}')
  node_record=$(echo "${DNS_RECORD}" |
    jq -r --arg ACTION "CREATE" \
          --arg CLUSTER_NAME "$cluster_name" \
          --arg VM_NAME "${all_hostnames[$node]}" \
          --arg TYPE "PTR" \
          --arg CLUSTER_DOMAIN "${cluster_domain}" \
          --arg IP_ADDRESS "${reversed_ipv4}.in-addr.arpa." \
          '.Action = $ACTION |
           .ResourceRecordSet.Name = $IP_ADDRESS |
           .ResourceRecordSet.Type = $TYPE |
           .ResourceRecordSet.ResourceRecords[0].Value = $CLUSTER_NAME+"-"+$VM_NAME+"."+$CLUSTER_DOMAIN+"."')
  ROUTE53_CREATE_JSON=$(echo "${ROUTE53_CREATE_JSON}" | jq --argjson DNS_RECORD "$node_record" -r '.Changes[.Changes|length] |= .+ $DNS_RECORD')
  node_record=$(echo "${node_record}" |
    jq -r --arg ACTION "DELETE" '.Action = $ACTION')
  ROUTE53_DELETE_JSON=$(echo "${ROUTE53_DELETE_JSON}" | jq --argjson DNS_RECORD "$node_record" -r '.Changes[.Changes|length] |= .+ $DNS_RECORD')
...
if [[ $IP_FAMILIES == *IPv6* ]]; then
  for (( node=0; node < ${#all_hostnames[@]}; node++)); do
    ...
    node_record=$(echo "${node_record}" |
      jq -r --arg ACTION "DELETE" '.Action = $ACTION')
    ROUTE53_DELETE_JSON=$(echo "${ROUTE53_DELETE_JSON}" | jq --argjson DNS_RECORD "$node_record" -r '.Changes[.Changes|length] |= .+ $DNS_RECORD')
    echo "Creating IPv6 REVERSE DNS entry for ${all_hostnames[$node]}"
    expanded_ipv6=$(echo ${all_ipv6Addresses[$node]} | sed 's/::/:0000:0000:0000:0000:0000:0000:0000:/' | sed 's/:0/:0000/g')
    reversed_ipv6=$(echo $expanded_ipv6 | sed 's/://g' | rev | sed 's/\(.\)/\1./g')
    node_record=$(echo "${DNS_RECORD}" |
      jq -r --arg ACTION "CREATE" \
            --arg CLUSTER_NAME "$cluster_name" \
            --arg VM_NAME "${all_hostnames[$node]}" \
            --arg TYPE "PTR" \
            --arg CLUSTER_DOMAIN "${cluster_domain}" \
            --arg IP_ADDRESS "${reversed_ipv6}ip6.arpa." \
            '.Action = $ACTION |
             .ResourceRecordSet.Name = ${IP_ADDRESS} |
             .ResourceRecordSet.Type = $TYPE |
             .ResourceRecordSet.ResourceRecords[0].Value = $CLUSTER_NAME+"-"+$VM_NAME+"."+$CLUSTER_DOMAIN+"."')
